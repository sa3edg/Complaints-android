...
public class AddNewComplaintView extends AbstractFragment {
	...
	public AddNewComplaintView(MainView mView) {
		super(mView);
	...
	public void onActivityCreated(Bundle savedState) {
		...
		// unitesSpinner.setOnItemSelectedListener(new
		// MyOnItemSelectedListener());
		complaintTitle = (EditText) activity
				.findViewById(R.id.complaintTitleTxt);
		locDescription = (EditText) activity.findViewById(R.id.locationDescTxt);
	...
	private void showAttachmentView() {
		mainView.displayAttachmentView(this, false);
	}

	private void showFIleBrowser() {
		mainView.displayFileBrowser(this, false, false);
	}

	private void showCameraView(boolean isVideo) {
		mainView.displayCameraView(this, null, isVideo);
	}
	...
	private void sendComplaint() {
		if (User.mySetting.getDefaultMobileNumber() == null) {
			...
			if (GenericUtils.isInternetConnected(mainView)) {
				...
				complaint.setLocationDescription(locDescription.getText()
						.toString());
				complaint.setLatitude(GenericUtils
						.roundFourDecimals(GPSLocationManager.latitude));
				complaint.setLongitude(GenericUtils
						.roundFourDecimals(GPSLocationManager.longitude));
				try {
		...
		ok.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (pNumber != null && !"".equals(pNumber)) {
					accessToken = pNumber;
					mplayerDialog.dismiss();
					activateNumber(AddNewComplaintView.this.phoneNumber,
							pNumber);
				}
			}
		});
	...
	private void showConfirmationDialog() {
		...
		title.setText(mainView.getResources().getString(
				R.string.save_complaint_dialog_title));
		TextView body = (TextView) mplayerDialog
				.findViewById(R.id.confirmationBody);
		body.setText(mainView.getResources().getString(
				R.string.save_complaint_dialog_body));
		final Button ok = (Button) mplayerDialog.findViewById(R.id.okBtn);
		// Set On ClickListener
		ok.setOnClickListener(new View.OnClickListener() {
	...
	private void saveComplaint() {
		Unit uni = new Unit();
		if (unites.size() > 0) {
			uni = unites.get(unitesSpinner.getSelectedItemPosition());
		}
		Complaint complaint = new Complaint();

		complaint.setTitle(complaintTitle.getText().toString());
		complaint.setDestinationUnit(uni.getName());
		complaint.setUnitId(uni.getId());
		...
		try {
			DBAdapter.addComplaintWIthAttachment(complaint);
			Toast.makeText(mainView,
					mainView.getResources().getString(R.string.saved_succeed),
	...
	private void attachFile(String complaintId, int index) {
		...
		try {
			...
			OrderCoordinator.handleOrder(
					this,
					RequestFactory.uploadFileRequest(complaintId,
							file.getPath()));
		} catch (Exception ex) {
	...
	public void postExecution(Response response) {
		...
		try {
			if (response.getRequestName().equals(Request.USER_LOC_ORDER)) {
				...
				String fileId = ResponseProcessingHelper.getInstance()
						.handleAttachFileResponse(response);
				if (fileId != null) {
					fileAttachIndex++;
				...
				String complaintId = ResponseProcessingHelper.getInstance()
						.handleAddComplaintResponse(response);
			...
			if (response.getRequestName().equals(Request.ADD_COMPLAINT_ORDER)
	...
	private void showError() {
		closeProgress();
		Toast.makeText(mainView,
				mainView.getResources().getString(R.string.connection_error),
				Toast.LENGTH_LONG).show();
	}
	...
	private void showSaveComplaintMsg() {
		closeProgress();
		Toast.makeText(mainView,
				mainView.getResources().getString(R.string.save_complaint_msg),
				Toast.LENGTH_LONG).show();
	}
